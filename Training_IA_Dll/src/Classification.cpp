...
Eigen::VectorXd Classification::prediction(const Eigen::MatrixXd& X) const
{
	// Y = X*W + b
...
void Classification::updateWeights(const Eigen::MatrixXd& X, const Eigen::VectorXd& Y) 
{
	//regle de rosenblatt
	...
	for (size_t iteration = 0; iteration < m_iteration; iteration++)
	{
		for (int i = 0; i < X.rows(); ++i)
		{
			...
			if (error != 0.0)
			{
				m_weight += m_learning_rate * error * X.row(i).transpose();
